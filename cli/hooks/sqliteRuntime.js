...
function ensureSqliteRuntime({ silent = false } = {}) {
  ...
  // Termux / Android has no native build toolchain and no Linux prebuild
  // compatible with its Android libc — skip the native compile entirely and use
  // the pure-JS sql.js fallback.
  const isTermux =
